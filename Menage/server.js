...
const { pool, init, PLACES, DEFAULT_PLACE } = require("./db");

// Resolve the requested place (from body or query), validated against the
// fixed list. Unknown/missing falls back to the default place.
function placeOf(req) {
  const raw = (req.body && req.body.place) || (req.query && req.query.place) || "";
  const t = String(raw).trim();
  return PLACES.includes(t) ? t : DEFAULT_PLACE;
}
...
// List all active actions for a place, grouped order by room/position.
app.get(
  "/api/actions",
  wrap(async (req, res) => {
    const { rows } = await pool.query(
      "SELECT id, label, room, position FROM actions WHERE active = TRUE AND place = $1 ORDER BY room, position, id",
      [placeOf(req)]
    );
    res.json(rows);
  })
...
app.post(
  ...
  wrap(async (req, res) => {
    ...
    await ensureRoom(place, room);
    const pos = await nextPosition(place, room);
    const { rows } = await pool.query(
      "INSERT INTO actions (place, label, room, position) VALUES ($1, $2, $3, $4) RETURNING id, label, room, position",
      [place, label, room, pos]
    );
  ...
  wrap(async (req, res) => {
    ...
    try {
      ...
      for (const id of orderedIds) {
        await client.query(
          "UPDATE actions SET position = $1 WHERE id = $2 AND room = $3 AND place = $4 AND active = TRUE",
          [pos++, id, room, place]
        );
      }
...
async function nextPosition(place, room) {
  const { rows } = await pool.query(
    "SELECT COALESCE(MAX(position), -1) + 1 AS p FROM actions WHERE place = $1 AND room = $2",
    [place, room]
  );
  return rows[0].p;
}
...
// Make sure a room exists for a place (added to the end if new).
async function ensureRoom(place, name) {
  await pool.query(
    "INSERT INTO rooms (place, name, position) VALUES ($1, $2, (SELECT COALESCE(MAX(position), -1) + 1 FROM rooms WHERE place = $1)) ON CONFLICT (place, name) DO NOTHING",
    [place, name]
  );
}

// List a place's rooms in display order (persists even with no tasks yet).
app.get(
  "/api/rooms",
  wrap(async (req, res) => {
    const { rows } = await pool.query(
      "SELECT name FROM rooms WHERE place = $1 ORDER BY position, name",
      [placeOf(req)]
    );
...
app.post(
  ...
  wrap(async (req, res) => {
    ...
    await ensureRoom(placeOf(req), name);
    res.status(201).json({ ok: true, name });
  })
);

// Reorder rooms within a place: rewrite positions 0..n in the given order.
app.put(
  "/api/rooms/reorder",
  wrap(async (req, res) => {
    const place = placeOf(req);
    ...
    try {
      ...
      for (const name of orderedNames) {
        await client.query("UPDATE rooms SET position = $1 WHERE name = $2 AND place = $3", [
          pos++,
          name,
          place,
        ]);
...
// Rename a room within a place (rooms table + library tasks + that place's snapshots).
app.put(
  "/api/rooms/rename",
  wrap(async (req, res) => {
    const place = placeOf(req);
    ...
    try {
      await client.query("BEGIN");
      await client.query("UPDATE actions SET room = $1 WHERE room = $2 AND place = $3", [
        newRoom,
        oldRoom,
        place,
      ]);
      await client.query(
        "UPDATE session_items SET room = $1 WHERE room = $2 AND session_id IN (SELECT id FROM sessions WHERE place = $3)",
        [newRoom, oldRoom, place]
      );
      // Keep the old room's position; if the new name already exists, this merges them.
      const posRow = await client.query("SELECT position FROM rooms WHERE name = $1 AND place = $2", [
        oldRoom,
        place,
      ]);
      await client.query("DELETE FROM rooms WHERE name = $1 AND place = $2", [oldRoom, place]);
      const position = posRow.rows.length
        ? posRow.rows[0].position
        : (
            await client.query(
              "SELECT COALESCE(MAX(position), -1) + 1 AS p FROM rooms WHERE place = $1",
              [place]
            )
          ).rows[0].p;
      await client.query(
        "INSERT INTO rooms (place, name, position) VALUES ($1, $2, $3) ON CONFLICT (place, name) DO NOTHING",
        [place, newRoom, position]
      );
      await client.query("COMMIT");
      res.json({ ok: true });
    } catch (e) {
...
// Delete a room within a place: remove it and soft-delete its tasks
...
app.delete(
  ...
  wrap(async (req, res) => {
    ...
    try {
      await client.query("BEGIN");
      await client.query("UPDATE actions SET active = FALSE WHERE room = $1 AND place = $2", [
        room,
        place,
      ]);
      await client.query("DELETE FROM rooms WHERE name = $1 AND place = $2", [room, place]);
      await client.query("COMMIT");
      res.json({ ok: true });
    } catch (e) {
      await client.query("ROLLBACK");
...
app.get(
  ...
  wrap(async (req, res) => {
    ...
    const sessions = (
      await pool.query(
        "SELECT id, to_char(session_date, 'YYYY-MM-DD') AS date, title FROM sessions WHERE place = $1 ORDER BY session_date ASC, id ASC",
        [place]
      )
    ).rows;
    const items = (
      await pool.query(
        `SELECT si.session_id, si.label, si.room, si.position, si.done
    ...
    const roomOrder = new Map(
      (await pool.query("SELECT name, position FROM rooms WHERE place = $1", [place])).rows.map(
        (r) => [r.name, r.position]
      )
    );
    const SEP2 = "|~|";
    const taskOrder = new Map(
      (
        await pool.query("SELECT room, label, position FROM actions WHERE active = TRUE AND place = $1", [
          place,
        ])
...
app.get(
  ...
  wrap(async (req, res) => {
    const s = await pool.query(
      "SELECT id, session_date, title FROM sessions WHERE session_date = $1 AND place = $2 ORDER BY id DESC LIMIT 1",
      [req.params.date, placeOf(req)]
    );
  ...
  wrap(async (req, res) => {
    ...
    try {
      ...
      let s = await client.query(
        "SELECT id FROM sessions WHERE session_date = $1 AND place = $2 ORDER BY id DESC LIMIT 1",
        [date, place]
      );
      ...
      if (!sessionId) {
        const ins = await client.query(
          "INSERT INTO sessions (place, session_date, title) VALUES ($1, $2, $3) RETURNING id",
          [place, date, title]
        );
      ...
      const toAdd = await client.query(
        "SELECT id, label, room, position FROM actions WHERE id = ANY($1::int[]) AND place = $2 ORDER BY room, position, id",
        [actionIds.filter((id) => !have.has(id)), place]
      );
